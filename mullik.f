      SUBROUTINE MULLIK(C,CBETA,UHF,H,NORBS,VECS,STORE)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INCLUDE 'SIZES'
      LOGICAL UHF
      DIMENSION C(*), CBETA(*), H(*), VECS(*), STORE(*)
**********************************************************************
*
...
*
**********************************************************************
      DIMENSION EIGS(120), IFACT(120), XYZ(3,NUMATM)
...
C  THE  +1.D-14 IS TO PREVENT ERRORS IN THE DIAGONALISATION.
                  STORE(IJ)=H(IJ)
   20          BJ=BETAP(NAT(J))
   30       CONTINUE
            DO 40 JJ=IF,K
   ...
   60 H(IFACT(I+1))=1.D0
      CALL HQRII(H,NORBS,NORBS,EIGS,VECS)
      DO 70 I=1,NORBS
   70 EIGS(I)=1.D0/SQRT(EIGS(I))
      IJ=0
      DO 90 I=1,NORBS
         DO 90 J=1,I
            IJ=IJ+1
...
*
      ...
      I=-1
      CALL DENSIT(VECS,NORBS,NORBS,NCLOSE,NOPEN,FRACT,C)
